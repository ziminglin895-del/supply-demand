...
export default function SupplyList() {
  ...
  useEffect(() => {
    fetchPosts({ type: 'supply', status: status || undefined, tag: tag || undefined, city: city || undefined })
  }, [status, tag, city])
    ...
    <div className="space-y-6">
      <div>
        <h1 className="font-['DM_Serif_Display'] text-3xl text-slate-100 max-sm:text-2xl">
          <span className="gradient-text-green">供给大厅</span>
        </h1>
        <p className="mt-1 text-sm text-slate-400">浏览所有已发布的IP供给信息</p>
      </div>
      ...
      <div className="glass-card flex flex-wrap items-center gap-3 p-4 max-sm:flex-col max-sm:items-stretch">
        ...
        <select
          ...
          className="rounded-lg border border-slate-700/50 bg-slate-800/50 px-3 py-2 text-sm text-slate-300 outline-none focus:border-emerald-500/50"
        >
      ...
      {loading ? (
        ...
        <div className="glass-card flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-slate-500">暂无供给信息</p>
        </div>
      ) : (
